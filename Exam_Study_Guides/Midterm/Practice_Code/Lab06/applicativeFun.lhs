...
>   Empty <*> _ = Empty
>   _ <*> Empty = Empty
>   (Node left right f) <*> (Node left' right' v) =
>      Node (left <*> left') (right <*> right') (f v)
...
This comes from the definition of Applicative Functors for lists.
...
Case #3: [1, 2, 3]
    [2, 3, 4]
    This is because it is the same as running:

> foo2 box = concat(map (\v -> [v+1]) box)