...
> applyOp Divide mi mj = mi >>= (\a -> mj >>= (\b -> if (b ==0) then Nothing else Just (a `div` b)))
...
>   j <- mj
>   return $ i + j
> applyOp' Minus mi mj = applyOp Plus mi (fmap (*(-1)) mj)
...
For this example, yet bind is the way to go.  It is makes more sense
since it is just doing successive operations onto the same data.  However,
this will not always apply as sometimes you may need to do something 
more complex like the applyOp example for which "do" is superior.
