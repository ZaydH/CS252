...
class AST
  ...
  def to_bytecode
    ...
    # YOUR CODE HERE -- Add 'when' cases to support the other expressions.
      when '+', '-', '*'
        op_string = ADD_OP if @op == '+'
        op_string = SUB_OP if @op == '-'
        op_string = MUL_OP if @op == '*'

        add_math_op(op_string, @args, bytecode)
  ...
  def add_math_op(op_string, numbers, bytecode)
    comp_arg(numbers[0], bytecode)
    (1..numbers.length - 1).each do |i|
      comp_arg(numbers[i], bytecode)
      bytecode.push(op_string)
    end
  end
end
