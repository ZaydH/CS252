...
> applyOp Divide mi mj = mi >>= (\i -> mj >>= (\j -> if j == 0 then Nothing else return (i `div` j)  ))
...
>                        i <- mi
>                        j <- mj
>                        if j == 0 then fail "Div by 0" else return (i `div` j)
...
> testDoubleHalf = Just 7 >>= mhalf >>= mdouble >>= mhalf >>= mdouble
